...
namespace Canducci.GraphQLQuery.Internals
{
   internal sealed class RulesExecute: IDisposable
   {
      ...
      {
         return string.Format(CultureInfo.InvariantCulture, "{0}{1}{2}{3}{4}", Signals.Backslashes, Signals.QuotationMark, ((DateTime)value).ToString("yyyy-MM-dd HH:mm:ss"), Signals.Backslashes, Signals.QuotationMark);
      }
      public string GetFormatTimeSpanAction(object value)
      {
         return string.Format(CultureInfo.InvariantCulture, "{0}{1}{2}{3}{4}", Signals.Backslashes, Signals.QuotationMark, ((TimeSpan)value).ToString(@"hh\:mm\:ss"), Signals.Backslashes, Signals.QuotationMark);
      }
